...
def make_tranx(event):
	...
	amount_sent = input_text4.GetValue()
	msg_sent = input_text6.GetValue()
	
	if not (username and private_key and receiver and amount_sent and msg_sent):
		wx.MessageBox('Please Fill In All Required Information', 'ERROR',wx.OK | wx.ICON_ERROR)
		sys.exit()
	else:
		try:
			check_amount = float(amount_sent)
			
			try:
				msg_len = len(msg_sent)
				if msg_len > 1025:
					print("Message length exceeds 1024 bytes")
				
				else:
					total_message = "3,#"+username+"#"+private_key+"#"+receiver+"#"+amount_sent+"#"+msg_sent
			...
			print("Please Enter Integer For Amount")
...
input_text2 = wx.TextCtrl(win, value = your_key, style = wx.TE_PASSWORD)
...
st3 = wx.StaticText(win, label = "Recipeient Name: ")
st3.SetFont(font)
...
st4 = wx.StaticText(win, label = "Transfer Amount: ")
st4.SetFont(font)
...
st5 = wx.StaticText(win, label = "Message: ")
st5.SetFont(font)
hbox5.Add(st5)
...
make_tx = wx.Button(win, label = 'Make Transaction', size = (150,30))
hbox7.Add(make_tx)
